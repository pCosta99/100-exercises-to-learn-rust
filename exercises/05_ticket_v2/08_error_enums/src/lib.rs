...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    Ticket::new(title.clone(), description, status.clone()).unwrap_or_else(|err| match err {
        TicketNewError::EmptyDescription | TicketNewError::DescriptionTooLong => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
        TicketNewError::EmptyTitle => panic!("Title cannot be empty"),
        TicketNewError::TitleTooLong => panic!("Title cannot be longer than 50 bytes"),
    })
}
